...
const PRESETS = [
  ...
  { label: '📊 Bollinger', slType: 'PERCENT', slValue: 3, tpType: 'BOLLINGER_BAND', tpValue: 8, bbPeriod: 20, bbStdDev: 2 },
...
export default function ConfigPanel({ config, onUpdate }) {
  const [form, setForm] = useState({ slType: 'PERCENT', slValue: 5, tpType: 'PERCENT', tpValue: 10, bbPeriod: 20, bbStdDev: 2 });
  ...
  const save = async () => {
    ...
    try {
      ...
      setMsg({ ok: true, text: 'Configuration saved! Takes effect within 30 seconds.' });
      onUpdate();
    } catch (e) {
      setMsg({ ok: false, text: 'Failed to save: ' + e.message });
    } finally { setSaving(false); }
  };

  const entryEx = 100;
  const slEx = form.slType === 'PERCENT' ? (entryEx * (1 - form.slValue / 100)).toFixed(2) : (entryEx - form.slValue).toFixed(2);
  const tpEx = form.tpType === 'PERCENT' ? (entryEx * (1 + form.tpValue / 100)).toFixed(2) : form.tpType === 'POINTS' ? (entryEx + form.tpValue).toFixed(2) : 'BB band';
